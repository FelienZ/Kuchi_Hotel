    ...
    <form method="post" action={{url('/booking/complete/'.$rooms['id'])}} class="flex flex-col bg-white drop-shadow-sm rounded-sm md:w-[60%] p-4 m-3 gap-4">
        ...
        <div class="detail grid sm:grid-cols-2 gap-2">
            <div class="checkin flex flex-col text-center gap-2 w-full">
                <p>Check-In:</p>
                <input type="date" name="cekin" disabled value="{{$checkin}}" class="input disabled:bg-blue-900 bg-blue-900 text-white disabled:text-white w-full">
                <input type="hidden" name="cekin" value="{{$checkin}}">
            </div>
            <div class="checkout flex flex-col text-center gap-2 w-full">
                <p>Check-Out:</p>
                <input type="date" name="cekout" disabled value="{{$checkout}}" class="input disabled:bg-blue-900 bg-blue-900 text-white disabled:text-white w-full">
